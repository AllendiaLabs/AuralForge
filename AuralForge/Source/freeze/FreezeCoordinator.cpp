...
#ifndef AURALFORGE_PYTHON_EXECUTABLE
#define AURALFORGE_PYTHON_EXECUTABLE "python3"
#endif
...
FreezeCoordinator::FreezeCoordinator(ArtifactPreparer artifactPreparer)
    : Thread("AuralForge Freeze Worker"),
...
void FreezeCoordinator::run() {
  ...
  auto requestFile = juce::File::createTempFile("auralforge-freeze.json");
  ...
  command.add(AURALFORGE_PYTHON_EXECUTABLE);
